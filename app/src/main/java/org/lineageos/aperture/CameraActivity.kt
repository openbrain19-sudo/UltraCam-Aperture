...
import org.lineageos.aperture.ui.views.SamsungModeSelectorLayout
...
open class CameraActivity : AppCompatActivity(R.layout.activity_camera) {
    ...
    private val samsungModeSelectorLayout by lazy { findViewById<SamsungModeSelectorLayout>(R.id.samsungModeSelectorLayout) }
        ...
        ZoomGestureDetector(this) {
            when (it) {
                ...
                is ZoomGestureDetector.ZoomEvent.Move -> {
                    try {
                        val method = viewModel.cameraController.javaClass.getDeclaredMethod(
                            "onPinchToZoom", Float::class.javaPrimitiveType
                        )
                        method.isAccessible = true
                        method.invoke(viewModel.cameraController, it.incrementalScaleFactor)
                    } catch (_: Exception) {}
    ...
    private val handler = object : Handler(Looper.getMainLooper()) {
        override fun handleMessage(msg: Message) {
            ...
            when (msg.what) {
                ...
                MSG_CLEAR_ZOOM_DRAG_FLAG -> {
                    isUserDraggingZoomSlider = false
                }
            }
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Set Samsung mode selector callback
        samsungModeSelectorLayout.onSamsungModeSelected = { mode ->
            viewModel.setSamsungShootingMode(mode)
        }
        ...
        launch {
            viewModel.cameraMode.collectLatest { cameraMode ->
                ...
                if (viewModel.samsungManagerInitialized) {
                    ...
                    samsungModeSelectorLayout.isVisible = false
                }
            }
        }
    ...
    private fun bindCameraUseCases(cameraConfiguration: CameraConfiguration) {
        ...
        val cameraUseCases = when (cameraConfiguration) {
            is CameraConfiguration.Photo -> {
                ...
                // imageOutputFormat not available in this CameraX version, skip
    ...
    companion object {
        ...
        private const val MSG_CLEAR_ZOOM_DRAG_FLAG = 3
